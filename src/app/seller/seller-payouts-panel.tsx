...
export function SellerPayoutsPanel() {
  ...
  async function loadStatus() {
    ...
    try {
      ...
      const payload = (await response.json().catch(() => null)) as PayoutStatus | { error?: string } | null;
      if (!response.ok) {
        throw new Error((payload as { error?: string } | null)?.error ?? "Could not load payout status.");
      }
  ...
  async function openExpressDashboard() {
    ...
    try {
      ...
      const payload = (await response.json().catch(() => null)) as { url?: string; error?: string } | null;
      if (!response.ok || !payload?.url) {
        throw new Error(payload?.error ?? "Could not open Stripe dashboard.");
      }
  ...
  async function savePayoutProfile(event: FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload?.error ?? "Could not save payout details.");
      }
